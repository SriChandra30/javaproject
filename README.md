...
Check logs (kubectl logs <pod> --previous) and describe (kubectl describe pod <pod>) to identify crash or liveness probe failure.
2.	A Kubernetes pod is stuck in a "Pending" state. What could be the possible reasons, and how would you troubleshoot it?
Insufficient resources, missing PVC, or unschedulable node — check with kubectl describe pod <pod>.
...
sudo systemctl start nagios / sudo systemctl stop nagios
...
Centralized monitoring, alerts, extensibility, and proactive issue detection.
